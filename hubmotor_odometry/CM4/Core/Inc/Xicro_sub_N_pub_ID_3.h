// ***************************************************************************************************************************************************//      |              This script was auto-generated by generate_library.py which received parameters from setup_xicro.yaml                   |//      |                                         EDITING THIS FILE BY HAND IS NOT RECOMMENDED                                                 |// ***************************************************************************************************************************************************#ifndef XICRO_SUB_N_PUB_ID_3_H#define XICRO_SUB_N_PUB_ID_3_H#include "stm32h7xx_hal.h"#include "string"#include "string.h"#include "math.h"class Xicro{
    public:
        Xicro();
        void begin(UART_HandleTypeDef* SerialObject);
        void Spin_node();void Test();
        
        // gen public pub void        void publish_nav_stm32(float pose__of__position__of__x ,float pose__of__position__of__y ,float pose__of__position__of__z ,float pose__of__orientation__of__x ,float pose__of__orientation__of__y ,float pose__of__orientation__of__z ,float pose__of__orientation__of__w ,float twist__of__linear__of__x ,float twist__of__linear__of__y ,float twist__of__linear__of__z ,float twist__of__angular__of__x ,float twist__of__angular__of__y ,float twist__of__angular__of__z );
        void publish_imu_stm32(float orientation__of__x ,float orientation__of__y ,float orientation__of__z ,float orientation__of__w ,float angular_velocity__of__x ,float angular_velocity__of__y ,float angular_velocity__of__z ,float linear_acceleration__of__x ,float linear_acceleration__of__y ,float linear_acceleration__of__z );
        // gen        struct{            float linear= 0;            float angular= 0;        } Sub_cmd_vel_stm32;        // gen state_srv_action        struct{        } State;        // gen struct srv client res        // gen service call void
    // get   
    private:
        uint8_t _Idmcu=3;
        UART_HandleTypeDef* _serial;
        uint8_t _getcrc(uint8_t crc,uint8_t *data,uint16_t len);
        const uint8_t _CRC_8_TABLE[256] = {};
        uint8_t _start[4]={73,109,64,99};
        uint8_t _stop[2]={126,126};
        uint8_t _continue[2]={42,42};
        void _Sendstart();
        void _SendSignature(uint8_t Idmcu,uint8_t Mode);
        void _Sendstop();
        void _Sendcontinue();
        void _SendUint8(uint8_t *data,uint8_t len);
        void _SendUint16(uint16_t *data,uint8_t len);
        void _SendUint32(uint32_t *data,uint8_t len);
        void _SendUint64(uint64_t *data,uint8_t len);
        void _SendInt8(int8_t *data,uint8_t len);
        void _SendInt16(int16_t *data,uint8_t len);
        void _SendInt32(int32_t *data,uint8_t len);
        void _SendInt64(int64_t *data,uint8_t len);
        void _SendFloat32(float *data,uint8_t len);     
        void _SendDouble(double *data,uint8_t len);
        void _SendString(std::string *data,uint8_t len);
        void _SendBool(bool *data,uint8_t len,bool flagNext);
        void _SendIdTopic(uint8_t IdTopic);

        uint8_t _crc=0;
        void _Sendcrc();
        uint8_t _state=0;
        uint8_t _datain[1]={0};
        uint64_t _bb=0;
        
        
        uint8_t _incomingIndexTopic=0;
        int8_t _OnTopic=-1;
        uint8_t _Ontype=0;
        uint8_t _Indexdata=0;
        uint8_t _crcIn=0;
       

        std::string _bufff="";
        uint8_t q=0;
        uint8_t w=0; 
        uint8_t _loop=0;
        uint8_t _loopTo=0;
        // gen        void* _nonverify[1][2][1];        void* _verify[1][2][1];        uint8_t _Idtopic_sub[1]={1};        uint8_t _TopicType[1][2]={{111,111}};        uint8_t _Totalvar[1]={2};        uint8_t _Nofdata[1][2]={{1, 1}};        // gen        struct{            float linear= 0;            float angular= 0;        }_Sub_cmd_vel_stm32;

        uint8_t _Onindex=0;
        uint8_t _Ongrab=0;
        uint8_t _OngrabTo=0;
        uint8_t _statetogo=0;

        uint8_t _UartBuff[64]={0};
        uint8_t _Posdata=0;
        uint8_t _Posdatapre=0;
};
#endif